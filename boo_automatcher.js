...
(function () {
  ...
  async function scanProfileImages() {
    ...
    try {
      ...
      if (profileImages.length === 0) {
        console.log("[Boo Automatcher] No profile images found to rate.");
        return;
      }
      let processedCount = 0;
      for (let img of profileImages) {
        if (!currentUser) currentUser = img.alt;
        if (await processSingleProfileImage(img)) processedCount++;
        await new Promise((resolve) => setTimeout(resolve, 200));
      }
      console.log(currentUser);
      console.log(`[Boo Automatcher] Successfully rated ${processedCount} images.`);
    } catch (error) {
  ...
  async function getCanvas(img) {
    ...
    try {
      ...
      const proxyResp = await fetch(proxyEndpoint, {
        ...
        body: JSON.stringify({ url: img.src, method: "GET" }),
      });
  ...
  async function predictImageBeauty(img) {
    ...
    try {
      ...
      console.error("[Boo Automatcher] Prediction error:", error);
      return 0;
    }
  }
